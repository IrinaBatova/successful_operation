...
def test_read_json_file_():
    with patch('json.load', return_value=[{"id": 1, "amount": 100}]) as mock_load:
...
def test_transaction_amount_usd(transactions_utils_usd):
...
class TestTransactionAmount(unittest.TestCase):
    ...
    def test_key_error(self):
        transaction = {'operationAmount': {}}  # Здесь отсутствует ключ 'amount'
